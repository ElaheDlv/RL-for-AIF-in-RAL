...
def _default_experiment_kwargs() -> Dict[str, Any]:
    ...
    for f in fields(Experiment):
        if f.default is not MISSING:
            defaults[f.name] = copy.deepcopy(f.default)
...
def _expand_param_grid(grid: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def _expand(node: Any) -> List[Any]:
        if isinstance(node, dict):
            combos = [{}]
            for key, value in node.items():
                options = _expand(value)
                new_combos: List[Dict[str, Any]] = []
                for combo in combos:
                    for opt in options:
                        new_combo = copy.deepcopy(combo)
                        new_combo[key] = copy.deepcopy(opt)
    ...
    expanded = _expand(grid)
    if not all(isinstance(item, dict) for item in expanded):
        raise ValueError("Parameter grid must expand to dictionaries")
    return expanded  # type: ignore[return-value]


def _load_experiments_from_config(config_path: Path) -> List[Experiment]:
    ...
    if not entries:
        raise ValueError("Config file must contain at least one experiment entry")

    experiments: List[Experiment] = []

    for entry in entries:
        if "name" not in entry:
            raise ValueError("Each experiment entry must have a 'name'")

        base_kwargs = _default_experiment_kwargs()
        _merge_dicts(base_kwargs, defaults)

        entry_copy = {k: v for k, v in entry.items() if k != "param_grid"}
        _merge_dicts(base_kwargs, entry_copy)

        required = ("algo", "obs_mode")
        for key in required:
            if not base_kwargs.get(key):
                raise ValueError(f"Experiment '{base_kwargs.get('name')}' missing required field '{key}'")
        ...
        for combo in combinations:
            exp_kwargs = copy.deepcopy(base_kwargs)
            _merge_dicts(exp_kwargs, combo)

            # ensure unique name if using grid
            if param_grid:
                suffix = _hash_config(combo)
                exp_kwargs["name"] = f"{base_kwargs['name']}__grid-{suffix}"

            experiments.append(Experiment(**exp_kwargs))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--checkpoint-dir",
        default=None,
        help="Template for checkpoint directory (relative paths resolved inside run dir; use {run} placeholder)",
    )
